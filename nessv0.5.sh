...
select_dns_mode() {
  ...
  echo "  0) Back (keep current: ${DNS_MODE} - ${DNS_DESC})"
  echo "  1) ${DNS_LABEL_ICANN}"
  echo "  2) ${DNS_LABEL_HYBRID}"
  echo "  3) ${DNS_LABEL_EMERDNS}"
  echo " 99) Exit (to shell)"
  echo
  read -rp "Select DNS mode [0-3,99]: " d_choice
  case "$d_choice" in
    0|"")
      echo "Keeping current: ${DNS_MODE} (${DNS_DESC})"
      return 0
      ;;
    1)
      DNS_MODE="icann"
...
select_profile() {
  ...
  echo "  0) Back (keep current: $(profile_label "$PROFILE"))"
  ...
  echo " 99) Exit (to shell)"
  echo
  read -rp "Select profile [0-5,99]: " p_choice
  case "$p_choice" in
    0|"")
      echo "Keeping current: $(profile_label "$PROFILE")"
      return 0
      ;;
    1)
      PROFILE="pi3"
...
print_info() {
  # Legacy: kept for compatibility but not used in the simplified TUI.
  # The new header is handled by print_header() for all menus.
  :
}
...
logo() {
  local lines=(
"          NESS v0.5.0"
"          Bedrock WORM Schemas"
"          AuxPoW-anchored Privateness"
"          Multi-overlay infrastructure"
...
menu() {
  if [ "$#" -gt 0 ]; then
    local raw seq choice
    raw="$*"
    raw=${raw//,/ }
    for choice in $raw; do
      case "$choice" in
        0)
          # Back has no effect in non-interactive sequence mode
          ;;
        1)
          select_dns_mode ;;
        2)
  ...
  while true; do
    print_header
    echo "  0) Back (exit to shell)"
    echo "  1) Reality / DNS Mode"
    echo "  2) Select Profile"
    echo "  3) Build images"
    echo "  4) Start/stop stack"
    echo "  5) Show stack status"
    echo "  6) Tail stack logs"
    echo "  7) Test / Status menu"
